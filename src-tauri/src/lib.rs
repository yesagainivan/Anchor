...
pub struct ScheduleRequest {
    pub tasks: Vec<Task>,
    pub anchor_date: String, // YYYY-MM-DD
    pub anchor_task_ids: Vec<String>, // Tasks that end on the anchor date
}
...
fn calculate_backwards_schedule(request: ScheduleRequest) -> Result<Vec<ScheduledTask>, String> {
    let anchor_date = NaiveDate::parse_from_str(&request.anchor_date, "%Y-%m-%d")
        .map_err(|e| format!("Invalid anchor date: {}", e))?;
    ...
    let mut processing_queue: Vec<String> = request.anchor_task_ids.clone();

    // Initial pass: Schedule anchor tasks
    // They end on the anchor date.
    for task_id in &request.anchor_task_ids {
        if let Some(task) = task_map.get(task_id) {
            let end_date = anchor_date;
            let start_date = end_date - Duration::days(task.duration_days);
            scheduled_tasks.insert(task_id.clone(), ScheduledTask {
                id: task.id.clone(),
    ...
    for task_id in &request.anchor_task_ids {
        end_dates.insert(task_id.clone(), anchor_date);
    }
    ...
    let mut pending_consumers_count: HashMap<String, usize> = HashMap::new();
    ...
    // Note: Anchor tasks might have consumers? 
    // If an anchor task has consumers, it's not a true root, but the user forced it to be anchored.
    // We'll treat the anchor constraint as an overriding "consumer" with start_date = anchor_date + duration? No, end_date = anchor_date.
    
    let mut queue: Vec<String> = request.anchor_task_ids.clone();
    ...
    for task_id in &request.anchor_task_ids {
        computed_end_dates.insert(task_id.clone(), anchor_date);
    }
    ...
    while let Some(task_id) = queue.pop() {
        ...
        // Determine end date
        let end_date = *computed_end_dates.get(&task_id).ok_or(format!("End date not computed for {}", task_id))?;
        let start_date = end_date - Duration::days(task.duration_days);
        ...
        for provider_id in &task.dependencies {
             let entry = computed_end_dates.entry(provider_id.clone()).or_insert(NaiveDate::MAX);
             if start_date < *entry {
                 *entry = start_date;
             }
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![greet, calculate_backwards_schedule])
